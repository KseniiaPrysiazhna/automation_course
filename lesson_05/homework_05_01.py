...
def get_car_by_search_criteria(year, engine_volume, price):
  searched_cars = [
    (car_id, car_criteria)
    for car_id, car_criteria in car_data.items()
    if car_criteria[1] >= year
       and car_criteria[2] >= engine_volume
       and car_criteria[4] <= price
  ]

  sorted_cars = sorted(searched_cars, key=lambda x: x[1][4])

  return sorted_cars[:5]

search_criteria = (2017, 1.6, 36000)
results = get_car_by_search_criteria(*search_criteria)

print("Cars by search criteria (sorted by price):")
for car_id, criteria in results:
    print(f"{car_id}: year = {criteria[1]}, engine = {criteria[2]}, price = ${criteria[4]}")